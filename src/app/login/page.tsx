...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We push them to dashboard, or admin if they are admin
      if (email === 'admin@123' || email === 'admin@bethelresidency.com') {
        router.push('/admin');
      } else {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (result.user.email === 'admin@123' || result.user.email === 'admin@bethelresidency.com') {
        router.push('/admin');
      } else {
        router.push('/dashboard');
      }
  ...
  return (
    <div className="min-h-screen pt-32 pb-24 flex items-center justify-center bg-brand-offwhite">
      <div className="bg-white p-8 rounded-2xl shadow-xl shadow-black/5 border border-gray-100 max-w-md w-full mx-4">
